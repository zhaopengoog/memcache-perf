...
const char *gengetopt_args_info_help[] = {
  ...
  "  -G, --getq_size=INT           Number of get requests per GETQ (default=50).",
  "  -g, --getq_freq=float         Ratio of multi-get:get commands. (default='0.0' i.e. no multi get).",
    0
};
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case 'M':	/* Set polling max  */
			setup_socket_timers(atoi(optarg),0);
			break;
		  
        case 'm':	/* Set polling max  */
			setup_socket_timers(0,atoi(optarg));
			break;
		  
        case 'G':	/* Set getq size  */
          if (update_arg( (void *)&(args_info->getq_size_arg), 
               &(args_info->getq_size_orig), &(args_info->getq_size_given),
              &(local_args_info.getq_size_given), optarg, 0, "50", ARG_INT,
              check_ambiguity, override, 0, 0,
              "getq_size", 'G',
              additional_error))
            goto failure;
